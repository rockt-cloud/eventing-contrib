...
const (
	...
	MessagingRole      = "rocketmq-channel"
)
...
func MakeChannelServiceName(name string) string {
	return fmt.Sprintf("%s-rn-channel", name)
}
...
func MakeK8sService(rc *v1alpha1.RocketmqChannel, opts ...ServiceOption) (*corev1.Service, error) {
	// Add annotations
	svc := &corev1.Service{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:      MakeChannelServiceName(rc.ObjectMeta.Name),
			Namespace: rc.Namespace,
			Labels: map[string]string{
				MessagingRoleLabel: MessagingRole,
			},
			OwnerReferences: []metav1.OwnerReference{
				*kmeta.NewControllerRef(rc),
			},
		},
